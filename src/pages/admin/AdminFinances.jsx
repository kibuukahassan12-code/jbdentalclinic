...
export default function AdminFinances({ api, getStoredKey }) {
    ...
    const [payForm, setPayForm] = useState({ invoice_id: '', amount: '', payment_method: 'Cash', paid_at: new Date().toISOString().slice(0, 10) });
    ...
    useEffect(() => { loadProfitLoss(); loadCashbook(); loadExpenses(); loadPayments(); }, [loadProfitLoss, loadCashbook, loadExpenses, loadPayments]);

    // ── Refresh all finance data ──
    const refreshAll = () => Promise.all([loadPayments(), loadCashbook(), loadProfitLoss(), loadExpenses()]);
    ...
    const handlePaySave = async (e) => {
        e.preventDefault();
        if (!payForm.invoice_id || !payForm.amount) { setPayError('Invoice and amount are required'); return; }
        ...
        try {
            ...
            if (res.ok) {
                ...
                setPayError(data.error || 'Failed to save');
            }
    ...
    const filteredPayments = payments.filter(pay => {
        ...
        const matchSearch = !payFilter || patName.includes(payFilter.toLowerCase()) || String(pay.invoice_id).includes(payFilter) || String(pay.id).includes(payFilter);
        const matchMethod = !payMethodFilter || pay.payment_method === payMethodFilter;
    ...
    return (
        <div className="space-y-6">
            ...
            {/* View Toggle: Payments / Balance Sheet / Cashbook / Expenses */}
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
                <div className="flex gap-1 bg-black/40 p-1 rounded-xl w-fit flex-wrap">
                    <button onClick={() => setActiveView('payments')}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-1.5 ${activeView === 'payments' ? 'bg-[#7FD856] text-black shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
                ...
                <div className="flex gap-2">
                    {activeView === 'payments' && (
                        <Button onClick={() => { setPayForm({ invoice_id: '', amount: '', payment_method: 'Cash', paid_at: new Date().toISOString().slice(0, 10) }); setShowPayForm(!showPayForm); setPayError(''); }}
                            className="bg-[#7FD856] text-black hover:bg-[#6FC745] rounded-xl font-medium">
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                        ...
                        {PAYMENT_METHODS.map(method => {
                            ...
                            const methodPayments = filteredPayments.filter(p => p.payment_method === method);
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                        <div className="relative flex-1 max-w-xs">
                            ...
                            <input type="text" placeholder="Search by patient or invoice..." value={payFilter} onChange={e => setPayFilter(e.target.value)}
                    ...
                    {showPayForm && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-white/5 border border-[#7FD856]/20 p-6">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                                    <ArrowUpRight size={18} className="text-green-400" /> Record Payment
                                </h3>
                                <button onClick={() => setShowPayForm(false)} className="text-gray-400 hover:text-white"><X size={20} /></button>
                            </div>
                            {payError && <p className="text-red-400 text-sm mb-3 p-2 bg-red-500/10 rounded-lg">{payError}</p>}
                            <form onSubmit={handlePaySave} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="sm:col-span-2">
                                    <label className="block text-sm text-gray-400 mb-1">Invoice *</label>
                                    <select required value={payForm.invoice_id} onChange={e => setPayForm(f => ({ ...f, invoice_id: e.target.value }))}
                                ...
                                <div>
                                    <label className="block text-sm text-gray-400 mb-1">Amount (UGX) *</label>
                                    <input required type="number" min="1" value={payForm.amount} onChange={e => setPayForm(f => ({ ...f, amount: e.target.value }))}
                                ...
                                <div className="flex items-end">
                                    <Button type="submit" disabled={paySaving} className="w-full bg-[#7FD856] text-black hover:bg-[#6FC745] rounded-xl font-medium py-2.5">
                    ...
                    {/* Payment list */}
                    {payLoading ? (
                        <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-[#7FD856] border-t-transparent rounded-full animate-spin" /></div>
